...
class PeerNetwork extends EventEmitter {
	...
	receiveMessage(toAddress, serialized) {
		...
		if (messenger) {
			this.log.trace('[peer-network] Delivering message to "%s" of message: %s', toAddress, serialized);
	...
	send(toAddress, serialized) {
		if (this._messengers[toAddress]) {
			setImmediate(() => {
				...
				if (messenger) {
					this.log.trace('[peer-network] Sending message in-memory to "%s": %s', toAddress, serialized);

					messenger.receiveMessage(serialized);

					this.emit('sent', { toAddress, serialized, inMemory: true });
				} else {
					this.log.trace('[peer-network] Sending message over network to "%s": %s', toAddress, serialized);
